...
_RHINO_SYSTEM_PATHS = [
    ...
    r'C:\Program Files\Rhino 7\System',
]
...
def initialize():
    """Initialize the CLR and load the D2P_Core assembly.
    ...
    # detect Rhino by trying to import it directly.
    # do NOT import clr or System here — that would
    # auto-initialize pythonnet's CLR and prevent
    # rhinoinside from configuring its own runtime.
    try:
        import Rhino  # NOQA: E401
        _in_rhino = True
    except ImportError:
        _in_rhino = False
        _load_rhino_inside()
    ...
    # add lib dir to sys.path so pythonnet can find D2P_Core
    if _LIB_DIR not in sys.path:
        sys.path.append(_LIB_DIR)

    # check if D2P_Core is already loaded (e.g. via GH plugin)
    loaded = None
    for asm in System.AppDomain.CurrentDomain.GetAssemblies():
        if asm.GetName().Name == 'D2P_Core':
            loaded = asm
            break
    ...
    if loaded is not None:
        ...
        dll_path = os.path.join(_LIB_DIR, 'D2P_Core.dll')
        if not os.path.isfile(dll_path):
            raise FileNotFoundError(
                f'D2P_Core.dll not found at {dll_path}'
            )
        clr.AddReference('D2P_Core')
...
def _check_version(assembly):
    ...
    if net_version != PACKAGE_VERSION:
        warnings.warn(
            f'd2p_core expects D2P_Core.dll {PACKAGE_VERSION}'
